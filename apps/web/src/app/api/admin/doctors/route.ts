...
async function proxy(method: 'GET' | 'POST', payload?: unknown) {
  ...
  try {
    const response = await fetch(`${apiUrl}/admin/doctors`, {
      method,
    ...
    return NextResponse.json({ message: 'Admin doctor service is unavailable' }, { status: 503 });
  }
}
